...
function openNewIncident() {
  ...
  window.IncidentRecorderAI?.restoreSettings?.();
...
function recorderAiContext() {
  extractRecorderDetails(false);
  const manualFields = {};
  RECORDER_DETAIL_FIELDS.forEach(([id]) => { manualFields[id] = value(id); });
  return {
    rawNotes: value("newRawNotes"),
    category: value("newCategory"),
    subcategory: value("newSubcategory"),
    manualFields,
    existingRuleBased: recorderSections(),
    agentName: "Chloe",
    transcriptMode: "agent_voice_only"
  };
}

function normalizeAiStep(step) {
  return normalizeRecorderTerms(String(step || ""))
    .replace(/^[-*•]+\s*/, "")
    .replace(/^(?:troubleshooting step\s*\d*[:.-]?\s*)/i, "")
    .trim();
}

function recorderStepTokens(step) {
  const stop = new Set(["the","a","an","and","or","to","of","in","on","for","with","was","is","are","were","be","been","rep","caller","customer","user","advised","confirmed","checked","guided","instructed"]);
  return new Set(normalizeAiStep(step).toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\s+/).filter((word)=>word.length>2 && !stop.has(word)));
}

function recorderStepsSimilar(a,b) {
  const aa=recorderStepTokens(a), bb=recorderStepTokens(b);
  if(!aa.size || !bb.size)return false;
  let overlap=0; aa.forEach((token)=>{if(bb.has(token))overlap+=1;});
  return overlap/Math.min(aa.size,bb.size)>=0.55;
}

function mergeRecorderAiSteps(aiSteps, fallbackSteps) {
  const merged=unique((Array.isArray(aiSteps)?aiSteps:[]).map(normalizeAiStep).filter(Boolean));
  (fallbackSteps||[]).map(normalizeAiStep).filter(Boolean).forEach((step)=>{
    if(!merged.some((existing)=>recorderStepsSimilar(existing,step))) merged.push(step);
  });
  return merged;
}

function recorderAiFieldSupportedByTranscript(suggested, raw=value("newRawNotes")) {
  const normalize=(text)=>normalizeRecorderTerms(text).toLowerCase().replace(/[^a-z0-9]+/g," ").trim();
  const needle=normalize(suggested);
...
function applyRecorderAiResult(result={}) {
  const fieldSuggestions=result.fields && typeof result.fields==="object" ? result.fields : {};
  const allowedFields=new Set(["cribProgramId","programName","companyName","siteId","accountNumber"]);
  Object.entries(fieldSuggestions).forEach(([id,suggested])=>{
    const candidate=String(suggested||"").trim();
    if(!allowedFields.has(id) || value(id) || !candidate || !recorderAiFieldSupportedByTranscript(candidate))return;
    const el=$(id); if(el)el.value=candidate;
  });
  renderRecorderDetectedSummary();

  const base=recorderSections();
  const aiSteps=mergeRecorderAiSteps(result.troubleshooting_steps, base.steps);
  const sections={
    ...base,
    issue: value("newSubcategory") || base.issue,
    issueSummary: normalizeRecorderTerms(result.issue_summary || base.issueSummary || ""),
    steps: aiSteps.length ? aiSteps : base.steps,
    resolution: normalizeRecorderTerms(result.resolution || base.resolution || "")
  };
  ...
  const subcategory=value("newSubcategory");
  const looksAutoGenerated=!currentTitle || titleEl?.dataset?.autoGenerated==="true" || (subcategory && (currentTitle===subcategory || currentTitle.startsWith(`${subcategory} - `)));
  if(looksAutoGenerated && titleEl){titleEl.value=buildRecorderShortDescription(sections);titleEl.dataset.autoGenerated="true";}

  $("detailedDescription").value=buildRecorderDetailedDescription(sections);
  $("workNotes").value=buildRecorderWorkNotes(sections);
  const ticket=`Short Description:\n${value("newTitle",buildRecorderShortDescription(sections))}\n\nDetailed Description:\n${value("detailedDescription")}\n\nWork Notes:\n${value("workNotes")}`;
  $("generatedTicket").value=ticket;
  $("recorderTimeSaved").textContent=`Generated with Local AI ${new Date().toLocaleTimeString([], {hour:"numeric", minute:"2-digit"})}`;
  setRecorderSaveStatus("Generated with Local AI");
  updateRecorderCounters();
  showToast("Ticket generated with local AI and verified against the rule-based notes.");
  return sections;
}

async function generateTicketWithAi() {
  const useAi=$("useLocalAi")?.checked;
  if(!useAi){generateTicketFromForm();return;}
  if(!value("newRawNotes")){
    showToast("Add rough notes before generating a ticket.");
    $("newRawNotes")?.focus();
    return;
  }
  const btn=$("generateTicketBtn");
  if(btn){btn.disabled=true;btn.setAttribute("aria-busy","true");}
  try{
    const ai=window.IncidentRecorderAI;
    if(!ai?.generateTicket) throw new Error("Local AI module is not ready.");
    const result=await ai.generateTicket(recorderAiContext());
    applyRecorderAiResult(result);
  }catch(error){
    console.warn("Local AI generation failed; using rule-based fallback.",error);
    generateTicketFromForm();
    const status=$("localAiStatus"); if(status && !/WebGPU|failed|unavailable/i.test(status.textContent||"")) status.textContent="AI unavailable · fallback used";
    showToast("Local AI was unavailable, so the rule-based generator was used.");
  }finally{
    if(btn){btn.disabled=false;btn.removeAttribute("aria-busy");}
  }
}

window.IncidentRecorderCore={
  recorderAiContext,
  applyRecorderAiResult,
  generateRuleBasedTicket:generateTicketFromForm,
  isOnsiteCategory:isRecorderOnsiteCategory
};
...
function wireEvents() {
  ...
  $("generateTicketBtn").addEventListener("click",generateTicketWithAi);
  ...
  document.addEventListener("keydown",(event)=>{ if(!$("newIncidentDialog").open || !(event.ctrlKey||event.metaKey))return; if(event.key==="Enter"){event.preventDefault();generateTicketWithAi();} if(event.key.toLowerCase()==="s"){event.preventDefault();saveRecorderDraft();} if(event.shiftKey&&event.key.toLowerCase()==="c"){event.preventDefault();copyText($("generatedTicket").value,"Generated ticket copied.");} });
